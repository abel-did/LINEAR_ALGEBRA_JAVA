...
public class Scene 
{
    ...
    public final int largeurPix=600;
    public final int hauteurPix=600;
    ...
    private BufferedImage imSphere;
    
    private MouseListener mouseListener;
    
    /**
     * Créé la scene et l'affiche dans une fenêtre
     */
    public Scene()
    {
        JFrame frame = new JFrame("Plan2D");
        frame.setSize(largeurPix+7,hauteurPix+29);
        ...
        try {
            imSphere = (BufferedImage)ImageIO.read(new File("sphere.png"));
        } catch (IOException e) {}
        ...
        panel.addMouseMotionListener(new MouseMotionListener(){
            ...
            public void mouseMoved(MouseEvent e){
                ...
                {
                     double u = (mouseX/(double)largeurPix)*Math.PI-Math.PI/2.0;
                     double v = (mouseY/(double)hauteurPix)*Math.PI-Math.PI/2.0;
                     mouseListener.deplacementObservateur(u,v);
                }
            }
    ...
    private class MonJPanel extends JPanel
    {
         ...
         public void paint(Graphics g) 
         {
             ...
             if(imSphere!=null)
             {
                int imX=imSphere.getWidth();
                int imY=imSphere.getHeight();
                g.setColor(Color.BLACK);
                g.drawRect(0,0,imX,imY);
                g.drawImage(imSphere,0,0,null);    
                
                double u = (mouseX/(double)largeurPix)*Math.PI-Math.PI/2.0;
                double v = (mouseY/(double)hauteurPix)*Math.PI-Math.PI/2.0;
                
                double cx=(Math.sin(u)*Math.cos(v))*imX/2.0+imY/2.0;
                double cy=(Math.sin(v))*imY/2.0+imY/2.0;
                
                int radius=5;
                
                g.setColor(Color.BLUE);
                g.fillArc((int)(cx-radius+0.5),(int)(cy-radius+0.5),radius*2,radius*2,0,360);
             }
